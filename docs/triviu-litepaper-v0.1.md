...
4. **No promises.** No return projections, in any material. Possibility is not probability — and Section 6 documents why. A fee charged on a result is the opposite of a promise — but "the result" is not the same thing on both of Triviu's paths, and saying otherwise would be a projection by omission. On the **arbitrage cycle**, the protocol earns only when the user does, enforced by the contract's own balance check. On **liquidity provision**, the fee is a share of the fees a position *collected*, and collected fees are not the owner's result: impermanent loss and recentering are subtracted afterwards, so a position can pay the fee and still end below what went into it. Section 4.6 states both, with the number we measured.
...
### 4.6 Success fee (on-chain)

**On the arbitrage cycle,** the protocol sustains itself with a **success fee**: a percentage of a cycle's **profit only**, taken inside the same atomic transaction and routed to a public treasury before the remainder returns to the caller. There is no entry or setup fee. A reverted cycle and a break-even cycle pay **nothing** — the fee exists only where real profit above gas exists. On this path, "we only earn if you earn" is enforced by code rather than asserted in copy: the fee branch is unreachable unless `finalBalance >= principal + minProfit` already holds.

**On liquidity provision, that sentence does not hold, and this is where we say so.** The LP vault's fee is a share of the fees a position **collected** — never of the principal, and nothing at all if the position collected nothing. But collected fees are not the same as the owner's result. What the owner keeps is collected fees *minus* impermanent loss *minus* the cost of recentering the range. Those subtractions happen after the fee is taken, so a position can pay the fee and still end below what went into it. In a measurement over 15 Polygon pools and 75 range/pool combinations (a $500 position, seven 24-hour windows, from block 91,846,456), 45 combinations ended positive for the owner and **30 ended negative — with the fee collected in all 75**. The console shows impermanent loss next to the collected fees for exactly this reason. We have not found a way to make the LP fee conditional on the owner's net result inside a single transaction, because the loss is only realized on exit; until that changes, the honest statement is this paragraph and not the sentence above it.
