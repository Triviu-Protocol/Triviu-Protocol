...
$('#protocolStateSelect').addEventListener('change',event=>applyProtocolState(event.target.value));$('#protocolStateAction').addEventListener('click',()=>{const value=$('#protocolStateBanner').dataset.state,state=protocolStates[value];openSheet(`<h3>${state.title}</h3><p>${state.copy}</p><div class="kv"><span>Source</span><b>STATE LAB · LOCAL</b></div><div class="kv"><span>Execution</span><b>DISABLED</b></div><button class="btn primary u-8c97caa1" data-act="close|nav:settings">Open the lab</button>`)});
...
function renderSwatches(){$('#swatches').innerHTML=accents.map(color=>`<button class="swatch u-a311ec3a" data-color="${color}" aria-label="Use colour ${color}"></button>`).join('');$$('.swatch').forEach(btn=>btn.addEventListener('click',()=>applyAccent(btn.dataset.color)))}
...
(function(){
  ...
  function paintSwatches(){
    ...
    host.innerHTML=SW.map(function(x){
      var c=x[1]||readAcc();
      return '<div class="bk-sw"><i class="u-a4939a21"></i><span>'+esc(x[0])+'<br>'+esc(hex2rgb(c))+'</span></div>';
    }).join('');
  }
  paintSwatches();
